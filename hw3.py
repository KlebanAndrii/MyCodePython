...
# player.fill(WHITE)
player_imgs = [pygame.transform.scale(pygame.image.load(IMGS_PATH + '/' + file).convert_alpha(), (100, 40)) for file in listdir(IMGS_PATH)]
...
while is_working:
    ...
    if presed_key[K_DOWN] and not player_rect.bottom >= heigth:
        player_rect = player_rect.move(0, player_speed)

    if presed_key[K_UP] and not player_rect.top <= 0:
        player_rect = player_rect.move(0, -player_speed)

    if presed_key[K_RIGHT] and not player_rect.right >= width:
        player_rect = player_rect.move(player_speed, 0)

    if presed_key[K_LEFT] and not player_rect.left <= 0:
        player_rect = player_rect.move(-player_speed, 0)
